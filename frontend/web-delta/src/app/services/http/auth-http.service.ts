...
/**
 * Serviço que implementa métodos de comunicação Http com autenticação password.
 */
...
export class AuthHttpService {
  ...
  public getById<T>(url: string, paramId: Number): Promise<void | T> {
    const headers = this.getConfigHeaders();
      return this.formatURL(url).then(finalURL => {
        console.log('Method get: ' + finalURL);
        return this.buildRequestOptions(headers, new HttpConfigRequest()).then(options => {
   ...
   */
  public post<T extends BaseModel>(url: string, config?: HttpConfigRequest): Promise<any> {
    config = config ? config : new HttpConfigRequest();
    const headers = this.getConfigHeaders();
    return this.formatURL(url).then(finalURL => {
      console.log('Method post: ' + finalURL);
      return this.buildRequestOptions(headers, config).then(options => {
        return this.httpClient().post<T>(finalURL, options.body, options).toPromise().then(res => this.extractData<T>(res));
      });
    });
  }
